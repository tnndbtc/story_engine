"""
story_engine storage — PostgreSQL database for generated stories.

The crawler DB is READ-ONLY to story_engine. This module manages
story_engine's own PostgreSQL database for persisting generated content.
...
# PostgreSQL DSN for story_engine's own database
...
CREATE TABLE IF NOT EXISTS stories (
    ...
    channel         BIGINT NOT NULL DEFAULT 1,
...
CREATE TABLE IF NOT EXISTS story_sets (
    id          BIGSERIAL PRIMARY KEY,
    batch_ts    BIGINT NOT NULL UNIQUE,
    lang        TEXT NOT NULL DEFAULT 'zh',
    channel     BIGINT NOT NULL DEFAULT 2,
    status      TEXT NOT NULL DEFAULT 'running',
    created_at  BIGINT,
    partial     BIGINT NOT NULL DEFAULT 0,
    partial_formats TEXT NOT NULL DEFAULT '[]',
    profile_id  TEXT DEFAULT NULL,
    cluster_score_breakdown TEXT DEFAULT NULL
);

CREATE INDEX IF NOT EXISTS idx_story_sets_batch_ts  ON story_sets(batch_ts);
CREATE INDEX IF NOT EXISTS idx_story_sets_status    ON story_sets(status);
CREATE INDEX IF NOT EXISTS idx_story_sets_profile   ON story_sets(profile_id);

CREATE TABLE IF NOT EXISTS used_items (
    id              BIGSERIAL PRIMARY KEY,
    crawler_item_id BIGINT NOT NULL,
    crawler_url     TEXT NOT NULL,
    hotness_at_use  DOUBLE PRECISION NOT NULL,
    story_set_id    BIGINT NOT NULL REFERENCES story_sets(id),
...
CREATE TABLE IF NOT EXISTS event_memory (
    ...
    embedding_center TEXT,
    entities         TEXT,
    story_id         BIGINT,             -- soft ref: stories.id OR hierarchical_stories.id
    story_set_id     BIGINT,             -- soft ref: story_sets.id (gaps exist from historical deletes)
...
CREATE TABLE IF NOT EXISTS hierarchical_stories (
    ...
    generated_at             BIGINT,
    created_at               BIGINT,
    attractiveness_score     BIGINT DEFAULT NULL,
    attractiveness_breakdown TEXT DEFAULT NULL,
    trend_bonus              BIGINT DEFAULT NULL,
    final_score              BIGINT DEFAULT NULL,
    story_type               TEXT DEFAULT NULL,
    produce_tier             TEXT DEFAULT NULL
);
...
CREATE TABLE IF NOT EXISTS youtube_publish_log (
    ...
    story_set_id        BIGINT,             -- soft ref: story_sets.id (gaps exist from historical deletes)
    story_id            BIGINT,             -- soft ref: hierarchical_stories.id (see register_youtube_publish)
...
class _PGConn:
    ...
    Allows the rest of models.py (and stage4_assign.py) to keep the same
    conn.execute() / conn.executescript() / conn.commit() / conn.close() pattern
    ...
    def execute(self, sql: str, params=None):
        """Execute a single statement.  Returns a RealDictCursor (dict-style rows)."""
        cur = self._conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        cur.execute(sql, params)
        return cur

    def executescript(self, sql: str):
        """Execute multiple semicolon-separated DDL/DML statements."""
...
def get_connection() -> '_PGConn':
    """Get a connection to story_engine's own PostgreSQL database."""
    conn = psycopg2.connect(PG_DSN)
    return _PGConn(conn)


def _add_column_if_missing(conn: '_PGConn', table: str, column: str, col_def: str) -> None:
    """Add a column to a table only if it does not already exist (PostgreSQL migration helper)."""
    row = conn.execute(
...
def _migrate_timestamps(conn):
    """No-op in PostgreSQL — all epoch integers are stored correctly from the migration."""
    return
...
def complete_story_set(set_id: int, status: str = 'complete'):
    ...
    conn = get_connection()
    conn.execute(
        "UPDATE story_sets SET status = %s WHERE id = %s",
        (status, set_id)
    )
    conn.commit()
    conn.close()
...
def save_cluster_score_breakdown(set_id: int, breakdown: list[dict]) -> None:
    ...
    try:
        conn = get_connection()
        conn.execute(
            "UPDATE story_sets SET cluster_score_breakdown = %s WHERE id = %s",
...
def get_platform_counts_for_set(story_set_id: int) -> dict[str, int]:
    ...
    rows = conn.execute(
        """SELECT platform, COUNT(*) as cnt
           FROM used_items
           WHERE story_set_id = %s AND platform IS NOT NULL AND (role = 'main' OR role IS NULL)
           GROUP BY platform""",
        (story_set_id,)
    ).fetchall()
    conn.close()
...
def get_story_sets(
    ...
    if profile_id is not None:
        conditions.append('ss.profile_id = %s')
        params.append(profile_id)
    if lang is not None:
        conditions.append('ss.lang = %s')
        params.append(lang)
    ...
    rows = conn.execute(
        f"""SELECT ss.*,
           ...
           {where_clause}
           GROUP BY ss.id
           HAVING COUNT(CASE WHEN s.status = 'ready' THEN 1 END) > 0
               OR COUNT(DISTINCT CASE WHEN hs.status = 'ready' THEN hs.id END) > 0
           ORDER BY ss.id DESC
           LIMIT %s""",
        params
    ).fetchall()
...
def get_stories_by_set(set_id: int) -> list[dict]:
    ...
    rows = conn.execute(
        "SELECT * FROM stories WHERE batch_id = %s AND status = 'ready' ORDER BY id",
        (set_id,)
    ).fetchall()
    ...
    hier_rows = conn.execute(
        "SELECT * FROM hierarchical_stories WHERE story_set_id = %s AND status = 'ready' ORDER BY id",
        (set_id,)
    ).fetchall()
    conn.close()
...
def save_attractiveness_score(
    ...
    conn.execute(
        """UPDATE hierarchical_stories
           SET attractiveness_score = %s, attractiveness_breakdown = %s,
               story_type = COALESCE(%s, story_type)
           WHERE story_set_id = %s""",
...
def save_trend_score(
    ...
    conn.execute(
        """UPDATE hierarchical_stories
           SET trend_bonus = %s, final_score = %s, produce_tier = %s
           WHERE story_set_id = %s""",
...
def force_tier_skip(story_set_id: int) -> None:
    ...
    conn = get_connection()
    conn.execute(
        "UPDATE hierarchical_stories SET produce_tier = 'skip' WHERE story_set_id = %s",
        (story_set_id,),
    )
    conn.commit()
    conn.close()
...
def save_failed_hierarchical_story(
    ...
    cursor = conn.execute(
        ...
        INSERT INTO hierarchical_stories
            (story_set_id, batch_ts, lang, channel, status,
             deep_story, supporting_stories, generated_at, created_at)
        VALUES (%s, %s, %s, %s, 'failed', NULL, NULL, %s, %s)
        RETURNING id
        """,
...
def get_story(story_id: int) -> dict | None:
    ...
    conn = get_connection()
    row = conn.execute("SELECT * FROM stories WHERE id = %s", (story_id,)).fetchone()
    conn.close()
    if row is None:
        return None
...
def get_stories_today(lang: str | None = None) -> list[dict]:
    ...
    if lang:
        rows = conn.execute(
            "SELECT * FROM stories WHERE generated_at >= %s AND lang = %s ORDER BY id",
            (today_start, lang),
        ).fetchall()
    else:
        rows = conn.execute(
            "SELECT * FROM stories WHERE generated_at >= %s ORDER BY id",
            (today_start,),
        ).fetchall()
    conn.close()
...
def get_stories(
    ...
    if date:
        ...
        conditions.append("generated_at >= %s AND generated_at < %s")
    ...
    conn = get_connection()
    rows = conn.execute(
        f"SELECT * FROM stories WHERE {where} ORDER BY generated_at DESC LIMIT %s",
        params + [limit],
    ).fetchall()
    conn.close()
    return [_row_to_dict(r) for r in rows]
...
def load_recent_events(window_days: int = 7) -> list[dict]:
    ...
    conn = get_connection()
    rows = conn.execute(
        "SELECT * FROM event_memory WHERE expires_at > %s ORDER BY created_at DESC",
        (now,),
    ).fetchall()
    conn.close()
    result = []
...
def register_youtube_publish(
    ...
    Returns the new youtube_publish_log row id (or 0 on INSERT DO NOTHING no-op).
    ...
    if story_id is None and story_set_id is not None:
        hs_row = conn.execute(
            "SELECT id FROM hierarchical_stories WHERE story_set_id = %s LIMIT 1",
            (story_set_id,),
        ).fetchone()
        if hs_row:
            story_id = hs_row['id']
...
def upsert_subscriber(
    ...
    ON CONFLICT (channel_id) DO UPDATE: always overwrites with latest data on re-fetch.
    """
    conn = get_connection()
    conn.execute(
        """INSERT INTO youtube_subscribers
           ...
           ON CONFLICT (channel_id) DO UPDATE SET
               ...
               subscribed_to    = EXCLUDED.subscribed_to,
               public_playlists = EXCLUDED.public_playlists,
...
def upsert_video_comment(
    ...
    Called by fetch_video_comments.py. ON CONFLICT (comment_id) DO UPDATE overwrites on re-fetch.
    """
    conn = get_connection()
    conn.execute(
        """INSERT INTO youtube_video_comments
...
def get_stories_with_comments() -> list[dict]:
    ...
    video_rows = conn.execute(
        """SELECT DISTINCT ypl.video_id, ypl.lang, ypl.upload_profile,
                  ...
                  hs.deep_story::jsonb->>'title' as story_title
