...
generate_stories() {
    ...
    echo ""
    echo "   1) All formats"
    echo "   2) Explainer    (A - 60秒解读)"
    echo "   3) Top 5        (B - 今日热点5)"
    echo "   4) Radar        (C - 全球雷达)"
    echo "   5) Regional     (D - 区域视角)"
    echo "   6) Two Takes    (E - 双面观点)"
    echo "   7) Pattern      (F - 趋势分析)"
    echo "   8) Viral        (G - 即将爆火)"
    echo "   9) Deep Dive    (H - 深度报道)"
    echo "  10) Niche        (I - 专题聚焦)"
